...
<div class="wrap">
    ...
    <?php if ($current_tab == 'apartments'): ?>
        ...
        if ($edit_id > 0) {
            ...
            ?>
            <h3>Listino: <?php echo ucfirst($apt->name); ?></h3><a href="<?php echo $base_url.'&tab=apartments'; ?>" class="button">« Torna</a>
    ...
    <?php else: ?>
        ...
        <table class="wp-list-table widefat fixed striped table-view-list">
            <thead><tr><th style="width:70px;">Stato</th><th>Date (Richiesta / Distinta)</th><th>Ospite</th><th>Note</th><th>Storico</th><th>Dettagli Soggiorno</th><th>Azioni</th></tr></thead>
            <tbody>
                <?php if(empty($rows)):?><tr><td colspan="7">Nessuna prenotazione.</td></tr><?php else: foreach($rows as $r): 
                    ...
                    $rc_icon = ($r->receipt_url)?'📄':''; $dt_req = date('d/m H:i', strtotime($r->created_at)); $dt_rec = ($r->receipt_uploaded_at) ? date('d/m H:i', strtotime($r->receipt_uploaded_at)) : '-'; $hist_json = htmlspecialchars($r->history_log, ENT_QUOTES, 'UTF-8');
                ?>
                <tr>
                    <td><?php echo $st.' '.$rc_icon; ?></td>
                    <td style="font-size:12px;">Rich: <?php echo $dt_req; ?><br>Dist: <?php echo $dt_rec; ?></td>
                    <td><strong><?php echo esc_html($r->guest_name); ?></strong><br><small><?php echo esc_html($r->guest_email); ?><br><?php echo esc_html($r->guest_phone); ?></small></td>
                    <td style="max-width:150px;"><small style="font-style:italic;"><?php echo esc_html($r->guest_notes); ?></small></td>
